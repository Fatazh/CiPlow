...
const SESSION_COOKIE = 'pplow-session'
...
export async function deleteSession(event: H3Event) {
  ...
  if (token) {
    await prisma.session.deleteMany({ where: { token } }).catch((err) => console.error('[Auth DB Error]', err))
  }
...
export async function getUserFromSession(event: H3Event) {
  ...
  if (session.expiresAt < new Date()) {
    await prisma.session.delete({ where: { id: session.id } }).catch((err) => console.error('[Auth DB Error]', err))
    deleteCookie(event, SESSION_COOKIE, { path: '/' })
    return null
  }
