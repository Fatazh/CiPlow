...
const prevLabel = computed(() => {
  // e.g. "Maret 2025" → derive nothing, just say "Bulan lalu"
  return 'Bln lalu'
})
...
function changeBadgeClass(row: RowDef): string {
  if (row.change === null) return 'bg-gray-100 text-gray-400 dark:bg-gray-800 dark:text-gray-500'
...
<template>
  ...
  <div v-if="loading" class="card rounded-2xl p-4 animate-pulse space-y-4">
    ...
    />
    <!-- Footer -->
    <div class="h-4 w-36 bg-gray-100 dark:bg-gray-800 rounded-full mx-auto" />
  </div>
  ...
  <div v-else class="card rounded-2xl p-4 animate-fade-in">
    ...
    <div class="flex items-center mb-2 px-1">
      ...
      <div class="flex items-center gap-3 flex-shrink-0">
        <span class="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wide w-16 text-right">
          Bln ini
        </span>
    ...
    <div class="space-y-0">
      <div
        ...
        :class="idx < rows.length - 1 ? 'border-b border-gray-100 dark:border-gray-800/80' : ''"
      >
        <!-- Icon bubble -->
        <div
          class="
