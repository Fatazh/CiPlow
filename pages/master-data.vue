...
useHead({ title: 'Master Data — PPLow' })
...
const walletLoading = computed(() => walletStatus.value === 'pending' && !walletsRaw.value)
...
const deleteWallet = async () => {
  ...
  try {
    await $fetch(`/api/wallets/${walletToDelete.value.id}`, { method: 'DELETE' })
...
const filteredCategories = computed(() => {
  let list = categories.value.filter((c: any) => c.type === categorySubTab.value)
...
const categoryLoading = computed(() => categoryStatus.value === 'pending' && !categoriesRaw.value)
...
const deleteCategory = async () => {
  ...
  try {
    await $fetch(`/api/categories/${categoryToDelete.value.id}`, { method: 'DELETE' })
...
const budgetData = computed(() => budgetsRaw.value?.data ?? { budgets: [], availableCategories: [] })
const budgetLoading = computed(() => budgetStatus.value === 'pending' && !budgetsRaw.value)
...
const deleteBudget = async () => {
  ...
  try {
    await $fetch(`/api/budgets/${budgetToDelete.value.id}`, { method: 'DELETE' })
...
watch(
  ...
  (open) => {
    if (open && budgetForm.amount > 0) {
      budgetAmountDisplay.value = budgetForm.amount.toLocaleString('id-ID')
    } else if (open) {
...
watch(
  ...
  (open) => {
    if (open && walletForm.balance > 0) {
      walletBalanceDisplay.value = walletForm.balance.toLocaleString('id-ID')
    } else if (open) {
...
<template>
  <div class="space-y-4 animate-fade-in">
    ...
    <!-- ── Page Title ──────────────────────────────────────────── -->
    <div>
      <h2 class="text-lg font-bold text-gray-800 dark:text-gray-100">Master Data</h2>
      <p class="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Kelola dompet, kategori & budget</p>
    </div>
    ...
    <div v-if="activeTab === 'wallet'" class="space-y-4">
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
        ...
        >
          <div class="w-12 h-12 rounded-2xl bg-gray-100 dark:bg-gray-800 flex items-center justify-center text-gray-400 text-2xl font-bold group-hover:bg-primary-500 group-hover:text-white transition-all duration-300 shadow-sm">+</div>
          <span class="text-[10px] font-black text-gray-400 group-hover:text-primary-500 uppercase tracking-widest">Tambah</span>
        </button>

        <!-- WALLET LIST -->
        <template v-if="walletLoading">
          <div v-for="i in 5" :key="i" class="aspect-square rounded-3xl bg-gray-100 dark:bg-gray-800 animate-pulse" />
        </template>
        ...
        <template v-else>
          <div
            ...
            :key="wallet.id"
            class="
              relative aspect-square rounded-[32px] p-5
              flex flex-col justify-between overflow-hidden
              transition-all duration-300 hover:shadow-2xl hover:-translate-y-1.5 active:scale-95
              cursor-pointer group shadow-md
            "
            :class="
              wallet.type === 'BANK' ? 'bg-gradient-to-br from-blue-500 to-blue-700 text-white' :
              wallet.type === 'CASH' ? 'bg-gradient-to-br from-emerald-500 to-emerald-700 text-white' :
              wallet.type === 'E_WALLET' ? 'bg-gradient-to-br from-sky-400 to-sky-600 text-white' :
              wallet.type === 'INVESTMENT' ? 'bg-gradient-to-br from-purple-500 to-purple-700 text-white' :
              'bg-gradient-to-br from-slate-500 to-slate-700 text-white'
            "
            @click="openWalletEdit(wallet)"
          >
            <!-- Even Larger Background Icon -->
            <span class="absolute -right-4 -bottom-6 text-[100px] opacity-25 pointer-events-none group-hover:scale-110 group-hover:-rotate-12 transition-all duration-500 leading-none">
              {{ walletIcons[wallet.type] ?? '💳' }}
            </span>

            <!-- Badge Default -->
            <div v-if="wallet.isDefault" class="absolute top-4 right-4 bg-white/30 backdrop-blur-md text-white text-[9px] font-black px-2 py-0.5 rounded-lg border border-white/40 z-10 uppercase tracking-tighter">Utama</div>

            <!-- Content Top -->
            <div class="relative z-10 pt-1">
              <p class="text-[10px] font-black uppercase tracking-[0.25em] opacity-80 mb-1.5">{{ walletTypeLabel(wallet.type) }}</p>
              <p class="text-lg font-extrabold leading-tight drop-shadow-md line-clamp-2">{{ wallet.name }}</p>
            </div>

            <!-- Content Bottom -->
            <div class="relative z-10 pb-1">
              <p class="text-xl font-bold leading-none tracking-tight drop-shadow-lg truncate">{{ formatIDR(wallet.balance) }}</p>
            </div>

            <!-- Action Overlay -->
            <div class="absolute inset-0 z-20 bg-black/30 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3 backdrop-blur-[3px]">
               <button v-if="!wallet.isDefault" @click.stop="setDefaultWallet(wallet.id)" class="w-10 h-10 flex items-center justify-center bg-white rounded-2xl text-lg shadow-xl transform translate-y-6 group-hover:translate-y-0 transition-all duration-300" title="Utama">⭐</button>
               <button @click.stop="confirmDeleteWallet(wallet)" class="w-10 h-10 flex items-center justify-center bg-rose-500 text-white rounded-2xl text-lg shadow-xl transform translate-y-6 group-hover:translate-y-0 transition-all duration-300 delay-75" title="Hapus">🗑️</button>
            </div>
          </div>
        </template>
      </div>
      ...
      <!-- Empty state -->
      <div v-if="!walletLoading && wallets.length === 0" class="py-10 text-center">
        <span class="text-4xl grayscale">👛</span>
        <p class="text-xs font-bold text-gray-400 mt-2">Dompet tidak ditemukan</p>
      </div>
    </div>
    ...
    <div v-if="activeTab === 'category'" class="space-y-4">
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
        ...
        <button
          class="
            aspect-square rounded-[32px] border-2 border-dashed
            border-gray-300 dark:border-gray-700
            flex flex-col items-center justify-center gap-2
            hover:border-primary-500 dark:hover:border-primary-500
            hover:bg-primary-50/10 transition-all duration-300
        ...
        <template v-if="categoryLoading">
          <div v-for="i in 5" :key="i" class="aspect-square rounded-[32px] bg-gray-100 dark:bg-gray-800 animate-pulse" />
        </template>
        ...
        <template v-else>
          ...
          >
            ...
            <span class="absolute -right-3 -bottom-5 text-[90px] opacity-20 pointer-events-none group-hover:scale-110 group-hover:-rotate-12 transition-all duration-500 leading-none">
              {{ cat.icon }}
            </span>

            <!-- Badge Default -->
            <div v-if="cat.isDefault" class="absolute top-4 right-4 bg-white/30 backdrop-blur-md text-white text-[8px] font-black px-1.5 py-0.5 rounded-lg border border-white/40 z-10 uppercase tracking-tighter">Default</div>

            <!-- Content Top -->
            <div class="relative z-10 pt-1">
              <p class="text-white text-base font-extrabold leading-tight drop-shadow-md line-clamp-2">{{ cat.name }}</p>
            </div>

            <!-- Content Bottom (Stats) -->
            <div class="relative z-10">
              <p class="text-white text-[9px] font-black uppercase tracking-widest opacity-80">
                {{ cat.transactionCount || 0 }} Transaksi
              </p>
            </div>

            <!-- Action Overlay -->
            <div class="absolute inset-0 z-20 bg-black/30 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3 backdrop-blur-[2px]">
               <button @click.stop="openCategoryEdit(cat)" class="w-10 h-10 flex items-center justify-center bg-white rounded-2xl text-lg shadow-xl" title="Edit">✏️</button>
               <button v-if="!cat.isDefault" @click.stop="confirmDeleteCategory(cat)" class="w-10 h-10 flex items-center justify-center bg-rose-500 text-white rounded-2xl text-lg shadow-xl" title="Hapus">🗑️</button>
            </div>
          </div>
        </template>
      </div>
      ...
      <!-- Empty state -->
      <div v-if="!categoryLoading && filteredCategories.length === 0" class="py-10 text-center">
        <span class="text-4xl grayscale">🏷️</span>
        <p class="text-xs font-bold text-gray-400 mt-2">Kategori tidak ditemukan</p>
      </div>
    </div>
    ...
    <div v-if="activeTab === 'budget'" class="space-y-3">
      ...
      >
        ...
        >+</span>
        <div class="text-left">
          <p class="text-sm font-semibold text-primary-600 dark:text-primary-400">Atur Budget</p>
          <p class="text-[11px] text-gray-400 dark:text-gray-500">
            {{ budgetData.availableCategories.length }} kategori tersedia
          </p>
        </div>
      </button>
      ...
      <div v-if="budgetLoading" class="space-y-3">
        <div v-for="i in 3" :key="i" class="card rounded-2xl p-4 animate-pulse space-y-3">
          <div class="flex items-center gap-3">
            <div class="w-9 h-9 rounded-xl bg-gray-200 dark:bg-gray-700" />
      ...
      >
        <div class="flex items-center gap-3 mb-3">
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-[11px] text-gray-400 dark:text-gray-500 mt-0.5">
              {{ formatIDR(budget.spent) }} / {{ formatIDR(budget.budgeted) }}
            </p>
          </div>
          <span
        ...
        <div class="flex items-center justify-between mt-2">
          <span class="text-[11px] text-gray-400 dark:text-gray-500">
            ...
            <span
              class="font-semibold"
              :class="budget.remaining >= 0 ? 'text-emerald-500' : 'text-rose-500'"
            >
      ...
      >
        ...
        <p class="text-xs text-gray-400 mt-1">Atur budget untuk mengontrol pengeluaran</p>
      </div>
    </div>
    ...
    <BottomSheet
      ...
      :title="walletFormMode === 'create' ? 'Tambah Dompet' : 'Edit Dompet'"
      :subtitle="walletFormMode === 'create' ? 'Buat dompet baru' : 'Perbarui informasi dompet'"
    >
      <div class="space-y-4">
        <!-- Name -->
        <div>
          <label class="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1.5 block">
            Nama Dompet
          </label>
          <input
        ...
        <div>
          ...
          <div class="grid grid-cols-3 gap-2">
            <button
              ...
              "
              @click="walletForm.type = wt.value; walletForm.icon = wt.icon"
            >
        ...
        <div class="flex items-center justify-between p-4 rounded-2xl bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-800">
          <div class="flex items-center gap-3">
            ...
            <div>
              <p class="text-sm font-bold text-gray-800 dark:text-gray-100">Dompet Utama</p>
              <p class="text-[10px] text-gray-400">Gunakan sebagai pilihan otomatis</p>
            </div>
          </div>
          <input
      ...
      <template #footer>
        ...
        >
          ...
          {{ walletFormMode === 'create' ? 'Tambah Dompet' : 'Simpan Perubahan' }}
        </button>
      </template>
    </BottomSheet>
    ...
    <BottomSheet
      ...
      :title="categoryFormMode === 'create' ? 'Tambah Kategori' : 'Edit Kategori'"
      :subtitle="categoryFormMode === 'create'
    ...
    >
      <div class="space-y-4">
        ...
        <div>
          ...
          <div class="flex flex-wrap gap-2">
            <button
              ...
              :key="c"
              class="w-8 h-8 rounded-full border-2 transition-all duration-150 active:scale-90"
              :class="categoryForm.color === c ? 'border-gray-800 dark:border-white scale-110' : 'border-transparent'"
        ...
        <div class="flex items-center justify-between p-4 rounded-2xl bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-800">
          <div class="flex items-center gap-3">
            <span class="text-xl">⭐</span>
            <div>
              <p class="text-sm font-bold text-gray-800 dark:text-gray-100">Kategori Default</p>
              <p class="text-[10px] text-gray-400">Gunakan sebagai pilihan otomatis untuk {{ categoryForm.type === 'EXPENSE' ? 'pengeluaran' : 'pemasukan' }}</p>
            </div>
          </div>
          <input
      ...
      <template #footer>
        ...
        >
          ...
          {{ categoryFormMode === 'create' ? 'Tambah Kategori' : 'Simpan Perubahan' }}
        </button>
      </template>
    </BottomSheet>
    ...
    >
      ...
      <template #footer>
        <button
          class="btn-primary w-full flex items-center justify-center gap-2"
          :disabled="(budgetFormMode === 'create' && !budgetForm.categoryId) || budgetForm.amount <= 0 || budgetSaving"
          @click="saveBudget"
        >
          <svg
            v-if="budgetSaving"
          ...
          {{ budgetFormMode === 'create' ? 'Simpan Budget' : 'Perbarui Budget' }}
        </button>
      </template>
    </BottomSheet>
