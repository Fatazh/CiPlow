...
useHead({ title: 'Riwayat Transaksi — PPLow' })
...
const deleteTx = async () => {
  ...
  try {
    await $fetch(`/api/transactions/${txToDelete.value.id}`, { method: 'DELETE' })
...
<template>
  <div class="space-y-4 animate-fade-in">
    ...
    <!-- ── Header ────────────────────────────────────────────── -->
    <div>
      <h2 class="text-lg font-bold text-gray-800 dark:text-gray-100">Riwayat Transaksi</h2>
      <p class="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Semua catatan keuanganmu</p>
    </div>
    ...
    <div class="grid grid-cols-3 gap-2">
      <div class="card rounded-2xl p-3 text-center">
        <p class="text-[10px] font-semibold text-gray-400 uppercase tracking-wide">Masuk</p>
        <p class="text-sm font-bold text-emerald-500 mt-1">
          {{ formatIDR(totals.income) }}
        </p>
      </div>
      <div class="card rounded-2xl p-3 text-center">
        <p class="text-[10px] font-semibold text-gray-400 uppercase tracking-wide">Keluar</p>
        <p class="text-sm font-bold text-rose-500 mt-1">
          {{ formatIDR(totals.expense) }}
        </p>
      </div>
      <div class="card rounded-2xl p-3 text-center">
        <p class="text-[10px] font-semibold text-gray-400 uppercase tracking-wide">Bersih</p>
        <p
          class="text-sm font-bold mt-1"
          :class="totals.net >= 0 ? 'text-emerald-500' : 'text-rose-500'"
        >
    ...
    <div v-if="loading" class="space-y-4">
      <div v-for="i in 3" :key="i" class="space-y-2">
        ...
        <div v-for="j in 2" :key="j" class="card rounded-2xl p-3.5 animate-pulse flex items-center gap-3">
    ...
    <div v-else-if="grouped.length > 0" class="space-y-4">
      <div v-for="group in grouped" :key="group.date">
        <!-- Date header -->
        <p class="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-2 px-0.5">
          {{ group.label }}
        </p>
        ...
        <div class="space-y-1.5">
          ...
          >
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-1.5 truncate">
                  ...
                  <span
                    v-if="tx.isPromo"
                    class="
                      flex-shrink-0 px-1.5 py-0.5 rounded-md
                ...
                <p class="text-[11px] text-gray-400 dark:text-gray-500 mt-0.5 truncate">
                  ...
                  <template v-else-if="tx.type === 'EXPENSE'">
                    {{ tx.walletFrom?.name ?? 'Unknown' }} · {{ tx.category.name }}
                    <span v-if="tx.quantity > 1 || tx.unitPrice > 0" class="ml-1 text-gray-300 dark:text-gray-600">|</span>
                    <span v-if="tx.quantity > 1 || tx.unitPrice > 0" class="ml-1 font-medium text-gray-500 dark:text-gray-400">
                      {{ tx.quantity }}x {{ formatIDR(tx.unitPrice || tx.amount) }}
                    </span>
                  </template>
                  <template v-else>
                    {{ tx.walletTo?.name ?? 'Unknown' }} · {{ tx.category.name }}
                  </template>
                </p>

                <!-- Promo Info Detail -->
                <p v-if="tx.isPromo" class="text-[10px] font-medium text-amber-500/80 mt-1 truncate italic">
                  <span v-if="tx.promoType === 'PERCENTAGE'">Diskon {{ tx.promoValue }}%</span>
                  <span v-else-if="tx.promoType === 'FIXED'">Potongan {{ formatIDR(tx.promoValue) }}</span>
                  <span v-else-if="tx.promoType === 'BUY_X_GET_Y'">{{ tx.promoDetails }}</span>
                </p>
              </div>
            ...
            <!-- Action buttons -->
            <div class="flex justify-end gap-3 mt-2 pt-2 border-t border-gray-50 dark:border-gray-800/50">
              <NuxtLink
    ...
    >
      ...
      <p class="text-xs text-gray-400 mt-1">
        {{ filter.type || filter.search ? 'Coba ubah filter pencarian' : `Untuk ${periodLabel}` }}
      </p>
      <NuxtLink
        to="/add-transaction"
        class="
